...
export const MyJobsApplication = ({ jobOffer, onCancel, onComplete }: Props) => {
  ...
  const cardHeader = (
    <div className="flex items-center justify-between h-[32px] -mt-2 mb-2">
      <Chip radius="sm" size="sm" variant="flat" color={chipColor}>
        {chipLabel}
      </Chip>
      {dropdownMenu.length > 0 && (
        <Dropdown isDisabled={isCanceling}>
          <DropdownTrigger>
            <Button aria-label="Job application options" variant="flat" radius="full" size="sm" isIconOnly>
              <Icon icon="dots" />
            </Button>
          </DropdownTrigger>
  ...
  return (
    <>
      <JobCard jobOffer={jobOffer} header={cardHeader} footer={cardFooter} />
