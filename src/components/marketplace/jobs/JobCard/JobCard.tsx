...
import { SKILL_EMOJIS } from '../../../../constants/marketplace/skills';
...
export const JobCard = (props: Props) => {
      ...
      <CardBody className="p-5">
        ...
        {!hideCreator && (
          <div className="flex items-center justify-between mb-2">
            {!shouldWrapInLink ? (
              <NextLink href={`/marketplace/profile/${creator._id}`} className="flex">
                {creatorBlock}
              </NextLink>
            ) : (
              creatorBlock
        ...
        <div className={styles.content}>
          {hasFooter && jobOfferId ? (
            <Link as={NextLink} href={`/marketplace/jobs/${jobOfferId}`} color="foreground" underline="hover">
              {cardTitle}
            </Link>
          ) : (
            cardTitle
  ...
  if (shouldWrapInLink) {
    return <NextLink href={`/marketplace/jobs/${jobOfferId}`}>{card}</NextLink>;
  }
