...
export const PostedJobs = ({ id }: Props) => {
  ...
  return (
    <>
      ...
      {shouldShowEmptyState && <EmptyState title="No jobs yet" text="There are no posted jobs yet. Get back later!" />}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {shouldShowSkeleton && <PostedJobsSkeleton />}
        {data?.map((jobOffer) => <JobCard key={jobOffer._id} hideCreator jobOffer={jobOffer} />)}
      </div>
    </>
  );
};
