...
import { requestUserConnections } from '../../../../services/marketplace/user';
...
export const Connections = ({ userId }: Props) => {
  ...
  const { isFetchingNextPage, isFetching, isLoading, data, hasNextPage, fetchNextPage } = useInfiniteQuery({
    ...
    getNextPageParam: (lastPage, allPages) => {
      return allPages.length * LIMIT < (lastPage?.connectionsCount ?? 0) ? allPages.length + 1 : undefined;
    },
  });
  ...
  return (
    <div className="container max-w-screen-lg mx-auto px-6">
      {isMyAccount && hasInvitations && (
        <>
          ...
          <div className="flex flex-col gap-4 mb-8">
            {invitations?.map((invitation) => <InvitationCard key={invitation._id} invitation={invitation} />)}
          </div>
        </>
      )}
      ...
      {shouldShowEmptyState && <EmptyState title="You don't have any connections yet" />}
    </div>
  );
};
