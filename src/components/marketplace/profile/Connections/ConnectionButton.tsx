...
export const ConnectionButton = ({ userId, inviteeName, hasConnection, hasInvitation, mode }: Props) => {
  ...
  if (hasConnection) {
    return (
      <Button
        as={NextLink}
        href={`/marketplace/chat/${userId}`}
        variant="flat"
        color="secondary"
        isIconOnly={isMobile}
        startContent={<Icon icon="chat" size={18} />}
      >
        {!isMobile && 'Message'}
      </Button>
    );
  ...
  return (
    <>
      <Tooltip
        ...
        content="You've reached your connection requests limit. Get to the next tier to increase the limit"
      >
        <div>
          <Button
            color="secondary"
      ...
      <InvitationModal userId={userId} inviteeName={inviteeName} isOpen={isOpen} onClose={onClose} />
    </>
  );
};
